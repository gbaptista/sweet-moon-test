package.preload["fennel.repl"] = package.preload["fennel.repl"] or function(...)
  ...
  local function completer(env, scope, text)
    ...
    local function add_partials(input, tbl, prefix)
      ...
      for k, is_mangled in utils.allpairs(_568_()) do
  ...
  local function reload(module_name, env, on_values, on_error)
    local _579_, _580_ = pcall(specials["load-code"]("return require(...)", env), module_name)
    if ((_579_ == true) and (nil ~= _580_)) then
      local old = _580_
      local _
      ...
      return on_values({"ok"})
    elseif ((_579_ == false) and (nil ~= _580_)) then
      local msg = _580_
      local function _585_()
        local _584_ = msg:gsub("\n.*", "")
        return _584_
      end
      return on_error("Runtime", _585_())
    else
      return nil
    end
  end
  local function run_command(read, on_error, f)
    local _587_, _588_, _589_ = pcall(read)
    if ((_587_ == true) and (_588_ == true) and (nil ~= _589_)) then
      local val = _589_
      return f(val)
    elseif (_587_ == false) then
  ...
  commands.reload = function(env, read, on_values, on_error)
    ...
    return run_command(read, on_error, _591_)
  end
  ...
  commands.complete = function(env, read, on_values, on_error, scope, chars)
    ...
    return run_command(read, on_error, _592_)
  end
  ...
  local function apropos_2a(pattern, tbl, prefix, seen, names)
    for name, subtbl in pairs(tbl) do
      if (("string" == type(name)) and (package ~= subtbl)) then
        local _593_ = type(subtbl)
        if (_593_ == "function") then
          if ((prefix .. name)):match(pattern) then
            table.insert(names, (prefix .. name))
          else
          end
        elseif (_593_ == "table") then
          if not seen[subtbl] then
            ...
            apropos_2a(pattern, subtbl, (prefix .. name:gsub("%.", "/") .. "."), _596_, names)
          else
          end
        else
        end
  ...
  commands.apropos = function(_env, read, on_values, on_error, _scope)
    ...
    return run_command(read, on_error, _601_)
  end
  ...
  local function apropos_doc(pattern)
    ...
    for _, path in ipairs(apropos(".*")) do
      ...
      if ("function" == type(tgt)) then
        local _605_ = (compiler.metadata):get(tgt, "fnl/docstring")
        if (nil ~= _605_) then
          local docstr = _605_
  ...
  commands["apropos-doc"] = function(_env, read, on_values, on_error, _scope)
    ...
    return run_command(read, on_error, _609_)
  end
  ...
  commands["apropos-show-docs"] = function(_env, read, on_values, on_error)
    ...
    return run_command(read, on_error, _611_)
  end
  ...
  local function resolve(identifier, _612_, scope)
    local _arg_613_ = _612_
    local ___replLocals___ = _arg_613_["___replLocals___"]
    local env = _arg_613_
    local e
    local function _614_(_241, _242)
      return (___replLocals___[_242] or env[_242])
    end
    e = setmetatable({}, {__index = _614_})
  ...
  commands.find = function(env, read, on_values, on_error, scope)
    local function _615_(_241)
      local _616_
      do
        local _617_ = utils["sym?"](_241)
        if (nil ~= _617_) then
          local _618_ = resolve(_617_, env, scope)
          if (nil ~= _618_) then
            _616_ = debug.getinfo(_618_)
          else
            _616_ = _618_
          end
        else
      ...
      if ((_G.type(_616_) == "table") and ((_616_).what == "Lua") and (nil ~= (_616_).linedefined) and (nil ~= (_616_).source) and (nil ~= (_616_).short_src)) then
        local line = (_616_).linedefined
        local source = (_616_).source
        local src = (_616_).short_src
        local fnlsrc
        do
          local t_621_ = compiler.sourcemap
          if (nil ~= t_621_) then
            t_621_ = (t_621_)[source]
          else
          end
          if (nil ~= t_621_) then
            t_621_ = (t_621_)[line]
    ...
    return run_command(read, on_error, _615_)
  end
  ...
  local function load_plugin_commands(plugins)
    for _, plugin in ipairs((plugins or {})) do
      for name, f in pairs(plugin) do
        local _629_ = name:match("^repl%-command%-(.*)")
        if (nil ~= _629_) then
          local cmd_name = _629_
  ...
  local function run_command_loop(input, read, loop, env, on_values, on_error, scope, chars)
    ...
    do
      local _631_ = commands[command_name]
      if (nil ~= _631_) then
        local command = _631_
  ...
  local function repl(options)
    ...
    local function _635_(parser_state)
      local c = byte_stream(parser_state)
      table.insert(chars, c)
      return c
    end
    read, reset = parser.parser(_635_)
    ...
    if opts.registerCompleter then
      local function _639_()
        local _637_ = env
        local _638_ = opts.scope
        local function _640_(...)
          return completer(_637_, _638_, ...)
        end
        return _640_
      end
      opts.registerCompleter(_639_())
    else
    end
    ...
    local function loop()
      ...
      elseif command_3f(src_string) then
        ...
        if parse_ok_3f then
          do
            local _644_, _645_ = nil, nil
            local function _647_()
              local _646_ = opts
              _646_["source"] = src_string
              return _646_
            end
            _644_, _645_ = pcall(compiler.compile, x, _647_())
            if ((_644_ == false) and (nil ~= _645_)) then
              local msg = _645_
              clear_stream()
              on_error("Compile", msg)
            elseif ((_644_ == true) and (nil ~= _645_)) then
              local src = _645_
              local src0
              if save_locals_3f then
              ...
              local _649_, _650_ = pcall(specials["load-code"], src0, env)
              if ((_649_ == false) and (nil ~= _650_)) then
                local msg = _650_
                clear_stream()
                on_error("Lua Compile", msg, src0)
              elseif (true and (nil ~= _650_)) then
                local _ = _649_
                local chunk = _650_
...
package.preload["fennel.specials"] = package.preload["fennel.specials"] or function(...)
  ...
  local function wrap_env(env)
    ...
    local function _364_(_, key, value)
      if utils["string?"](key) then
    ...
    local function _366_()
      local function putenv(k, v)
        ...
        if utils["string?"](k) then
          _367_ = compiler["global-unmangling"](k)
        else
          _367_ = k
        end
        return _367_, v
    ...
    return setmetatable({}, {__index = _362_, __newindex = _364_, __pairs = _366_})
  end
  local function current_global_names(_3fenv)
    local mt
    do
      local _369_ = getmetatable(_3fenv)
      if ((_G.type(_369_) == "table") and (nil ~= (_369_).__pairs)) then
        local mtpairs = (_369_).__pairs
        local tbl_11_auto = {}
        for k, v in mtpairs(_3fenv) do
          local _370_, _371_ = k, v
          if ((nil ~= _370_) and (nil ~= _371_)) then
            local k_12_auto = _370_
            local v_13_auto = _371_
  ...
  local function doc_2a(tgt, name)
    if not tgt then
      ...
      if ((type(tgt) == "function") or ((type(mt) == "table") and (type(mt.__call) == "function"))) then
        ...
        return string.format("(%s%s%s)\n  %s", name, _376_, arglist, docstring)
      else
  ...
  local function get_fn_name(ast, scope, fn_name, multi)
    if (fn_name and (fn_name[1] ~= "nil")) then
      local _393_
      if not multi then
        _393_ = compiler["declare-local"](fn_name, {}, scope, ast)
      else
        _393_ = (compiler["symbol-to-expression"](fn_name, scope))[1]
      end
      return _393_, not multi, 3
    else
      return nil, true, 2
    end
  end
  local function compile_named_fn(ast, f_scope, f_chunk, parent, index, fn_name, local_3f, arg_name_list, f_metadata)
    ...
    local _396_
    if local_3f then
      _396_ = "local function %s(%s)"
    else
      _396_ = "%s = function(%s)"
    end
    compiler.emit(parent, string.format(_396_, fn_name, table.concat(arg_name_list, ", ")), ast)
  ...
  local function get_function_metadata(ast, arg_list, index)
    ...
    if (utils["string?"](expr) and (index_2a < #ast)) then
      local _399_
      do
        local _398_ = f_metadata
        _398_["fnl/docstring"] = expr
        _399_ = _398_
      end
      return _399_, index_2a
    elseif (utils["table?"](expr) and (index_2a < #ast)) then
      local _400_
      do
        local tbl_11_auto = f_metadata
        for k, v in pairs(expr) do
          local _401_, _402_ = k, v
          if ((nil ~= _401_) and (nil ~= _402_)) then
            local k_12_auto = _401_
            local v_13_auto = _402_
            tbl_11_auto[k_12_auto] = v_13_auto
          else
          end
        end
  ...
  SPECIALS.fn = function(ast, scope, parent)
    local f_scope
    do
      local _405_ = compiler["make-scope"](scope)
      do end (_405_)["vararg"] = false
      f_scope = _405_
    end
    local f_chunk = {}
  ...
  SPECIALS.lua = function(ast, _, parent)
    ...
    local _409_
    do
      local _408_ = utils["sym?"](ast[2])
      if (nil ~= _408_) then
        _409_ = tostring(_408_)
      else
        _409_ = _408_
      end
    end
    ...
    local _413_
    do
      local _412_ = utils["sym?"](ast[3])
      if (nil ~= _412_) then
        _413_ = tostring(_412_)
      else
        _413_ = _412_
      end
    end
  ...
  local function dot(ast, scope, parent)
    ...
    local len = #ast
    local _let_416_ = compiler.compile1(ast[2], scope, parent, {nval = 1})
    local lhs = _let_416_[1]
    if (len == 2) then
      return tostring(lhs)
    else
      local indices = {}
      for i = 3, len do
        ...
        if (utils["string?"](index) and utils["valid-lua-identifier?"](index)) then
          ...
          local _let_417_ = compiler.compile1(index, scope, parent, {nval = 1})
          local index0 = _let_417_[1]
  ...
  local function disambiguate_3f(rootstr, parent)
    local function _426_()
      local _425_ = get_prev_line(parent)
      if (nil ~= _425_) then
        local prev_line = _425_
        return prev_line:match("%)$")
      else
        return nil
      end
    end
    return (rootstr:match("^{") or _426_())
  end
  SPECIALS.tset = function(ast, scope, parent)
    ...
    local keys = {}
    for i = 3, (#ast - 1) do
      local _let_428_ = compiler.compile1(ast[i], scope, parent, {nval = 1})
      local key = _let_428_[1]
  ...
  local function compile_until(condition, scope, chunk)
    if condition then
      local _let_437_ = compiler.compile1(condition, scope, chunk, {nval = 1})
      local condition_lua = _let_437_[1]
  ...
  local function native_method_call(ast, _scope, _parent, target, args)
    local _let_441_ = ast
    local _ = _let_441_[1]
    local _0 = _let_441_[2]
    local method_string = _let_441_[3]
    local call_string
  ...
  local function method_call(ast, scope, parent)
    ...
    local _let_443_ = compiler.compile1(ast[2], scope, parent, {nval = 1})
    local target = _let_443_[1]
    local args = {}
    for i = 4, #ast do
      local subexprs
      local _444_
      ...
      subexprs = compiler.compile1(ast[i], scope, parent, {nval = _444_})
  ...
  SPECIALS.hashfn = function(ast, scope, parent)
    ...
    local f_scope
    do
      local _449_ = compiler["make-scope"](scope)
      do end (_449_)["vararg"] = false
      _449_["hashfn"] = true
      f_scope = _449_
    end
    local f_chunk = {}
  ...
  local function maybe_short_circuit_protect(ast, i, name, _453_)
    local _arg_454_ = _453_
    local mac = _arg_454_["macros"]
  ...
  local function arithmetic_special(name, zero_arity, unary_prefix, ast, scope, parent)
    ...
    local _457_ = #operands
    if (_457_ == 0) then
      local _459_
      do
        local _458_ = zero_arity
        compiler.assert(_458_, "Expected more than 0 arguments", ast)
        _459_ = _458_
      end
      return utils.expr(_459_, "literal")
    elseif (_457_ == 1) then
  ...
  local function define_arithmetic_special(name, zero_arity, unary_prefix, _3flua_name)
    local _465_
    do
      local _462_ = (_3flua_name or name)
      local _463_ = zero_arity
      local _464_ = unary_prefix
      local function _466_(...)
        return arithmetic_special(_462_, _463_, _464_, ...)
      end
      _465_ = _466_
    end
    SPECIALS[name] = _465_
  ...
  local function bitop_special(native_name, lib_name, zero_arity, unary_prefix, ast, scope, parent)
    if (#ast == 1) then
      ...
      for i = 2, len do
        ...
        subexprs = compiler.compile1(ast[i], scope, parent, {nval = _467_})
        utils.map(subexprs, tostring, operands)
      end
      if (#operands == 1) then
  ...
  local function define_bitop_special(name, zero_arity, unary_prefix, native)
    ...
    do
      ...
      local function _478_(...)
        return bitop_special(_473_, _474_, _475_, _476_, ...)
      end
      _477_ = _478_
    end
    SPECIALS[name] = _477_
  ...
  local function native_comparator(op, _479_, scope, parent)
    local _arg_480_ = _479_
    local _ = _arg_480_[1]
    local lhs_ast = _arg_480_[2]
    local rhs_ast = _arg_480_[3]
    local _let_481_ = compiler.compile1(lhs_ast, scope, parent, {nval = 1})
    local lhs = _let_481_[1]
    local _let_482_ = compiler.compile1(rhs_ast, scope, parent, {nval = 1})
    local rhs = _let_482_[1]
  ...
  local function make_compiler_env(ast, scope, parent, _3fopts)
    local provided
    do
      local _490_ = (_3fopts or utils.root.options)
      if ((_G.type(_490_) == "table") and ((_490_)["compiler-env"] == "strict")) then
        provided = safe_compiler_env()
      elseif ((_G.type(_490_) == "table") and (nil ~= (_490_).compilerEnv)) then
        local compilerEnv = (_490_).compilerEnv
        provided = compilerEnv
      elseif ((_G.type(_490_) == "table") and (nil ~= (_490_)["compiler-env"])) then
        local compiler_env = (_490_)["compiler-env"]
  ...
  local _local_496_ = _497_(...)
  local dirsep = _local_496_[1]
  local pathsep = _local_496_[2]
  local pathmark = _local_496_[3]
  ...
  local function search_module(modulename, _3fpathstring)
    ...
    local function try_path(path)
      ...
      local _499_ = (io.open(filename) or io.open(filename2))
      if (nil ~= _499_) then
        local file = _499_
        file:close()
        return filename
    ...
    local function find_in_path(start, _3ftried_paths)
      local _501_ = fullpath:match(pattern, start)
      if (nil ~= _501_) then
        local path = _501_
        local _502_, _503_ = try_path(path)
        if (nil ~= _502_) then
          local filename = _502_
          return filename
        elseif ((_502_ == nil) and (nil ~= _503_)) then
          local error = _503_
          local function _505_()
            local _504_ = (_3ftried_paths or {})
            table.insert(_504_, error)
            return _504_
          end
          return find_in_path((start + #path + 1), _505_())
        else
          return nil
        end
      elseif true then
  ...
  local function make_searcher(_3foptions)
    local function _510_(module_name)
      local opts = utils.copy(utils.root.options)
      for k, v in pairs((_3foptions or {})) do
        opts[k] = v
      end
      opts["module-name"] = module_name
      local _511_, _512_ = search_module(module_name)
      if (nil ~= _511_) then
        local filename = _511_
        local _515_
        do
          local _513_ = filename
          local _514_ = opts
          local function _516_(...)
            return utils["fennel-module"].dofile(_513_, _514_, ...)
          end
          _515_ = _516_
        end
        return _515_, filename
  ...
  local function fennel_macro_searcher(module_name)
    local opts
    do
      local _518_ = utils.copy(utils.root.options)
      do end (_518_)["env"] = "_COMPILER"
      _518_["requireAsInclude"] = false
      _518_["allowedGlobals"] = nil
      opts = _518_
    end
    local _519_ = search_module(module_name, utils["fennel-module"]["macro-path"])
    if (nil ~= _519_) then
      local filename = _519_
      local _520_
      if (opts["compiler-env"] == _G) then
        local _521_ = fennel_macro_searcher
        local _522_ = filename
        local _523_ = opts
        local function _525_(...)
          return dofile_with_searcher(_521_, _522_, _523_, ...)
        end
        _520_ = _525_
      else
        local _526_ = filename
        local _527_ = opts
        local function _529_(...)
          return utils["fennel-module"].dofile(_526_, _527_, ...)
        end
        _520_ = _529_
      end
      return _520_, filename
    else
      return nil
    end
  end
  local function lua_macro_searcher(module_name)
    local _532_ = search_module(module_name, package.path)
    if (nil ~= _532_) then
      local filename = _532_
      local code
      do
        ...
        code = close_handlers_8_auto(_G.xpcall(_534_, (package.loaded.fennel or debug).traceback))
      end
  ...
  local function search_macro_module(modname, n)
    local _536_ = macro_searchers[n]
    if (nil ~= _536_) then
      local f = _536_
      local _537_, _538_ = f(modname)
      if ((nil ~= _537_) and true) then
        local loader = _537_
        local _3ffilename = _538_
  ...
  local function _542_(modname)
    ...
    return (macro_loaded[modname] or metadata_only_fennel(modname) or _543_())
  end
  safe_require = _542_
  ...
  local function resolve_module_name(_544_, _scope, _parent, opts)
    local _arg_545_ = _544_
    local filename = _arg_545_["filename"]
    local second = _arg_545_[2]
  ...
  local function include_path(ast, opts, path, mod, fennel_3f)
    ...
    do
      ...
      src = close_handlers_8_auto(_G.xpcall(_551_, (package.loaded.fennel or debug).traceback))
    end
  ...
  SPECIALS.include = function(ast, scope, parent, opts)
    ...
    local modexpr
    do
      local _554_, _555_ = pcall(resolve_module_name, ast, scope, parent, opts)
      if ((_554_ == true) and (nil ~= _555_)) then
        local modname = _555_
    ...
    if ((modexpr.type ~= "literal") or ((modexpr[1]):byte() ~= 34)) then
      ...
      local function _559_()
        local _558_ = search_module(mod)
        if (nil ~= _558_) then
          local fennel_path = _558_
      ...
      res = ((utils["member?"](mod, (utils.root.options.skipInclude or {})) and opts.fallback(modexpr, true)) or include_circular_fallback(mod, modexpr, opts.fallback, ast) or utils.root.scope.includes[mod] or _559_())
...
package.preload["fennel.compiler"] = package.preload["fennel.compiler"] or function(...)
  ...
  local function assert_compile(condition, msg, ast)
    if not condition then
      local _let_220_ = (utils.root.options or {})
      local source = _let_220_["source"]
      local unfriendly = _let_220_["unfriendly"]
  ...
  local function serialize_string(str)
    ...
    return string.gsub(string.gsub(string.format("%q", str), ".", serialize_subst), "[\128-\255]", _224_)
  end
  local function global_mangling(str)
    if utils["valid-lua-identifier?"](str) then
      ...
      return ("__fnl_global__" .. str:gsub("[^%w]", _225_))
    end
  end
  local function global_unmangling(identifier)
    local _227_ = string.match(identifier, "^__fnl_global__(.*)$")
    if (nil ~= _227_) then
      local rest = _227_
      local _228_
      ...
      _228_ = string.gsub(rest, "_[%da-f][%da-f]", _229_)
      return _228_
    elseif true then
      local _ = _227_
      return identifier
  ...
  local function local_mangling(str, scope, ast, _3ftemp_manglings)
    ...
    mangling = string.gsub(string.gsub(raw, "-", "_"), "[^%w_]", _233_)
  ...
  local function autogensym(base, scope)
    local _236_ = utils["multi-sym?"](base)
    if (nil ~= _236_) then
      local parts = _236_
    ...
    elseif true then
      ...
      return (scope.autogensyms[base] or _237_())
    else
      return nil
    end
  end
  ...
  local function make_metadata()
    local function _259_(self, tgt, key)
      if self[tgt] then
        return self[tgt][key]
      else
        return nil
    ...
    local function _261_(self, tgt, key, value)
      self[tgt] = (self[tgt] or {})
      do end (self[tgt])[key] = value
      return tgt
    end
    local function _262_(self, tgt, ...)
      local kv_len = select("#", ...)
      local kvs = {...}
      if ((kv_len % 2) ~= 0) then
    ...
    return setmetatable({}, {__index = {get = _259_, set = _261_, setall = _262_}, __mode = "k"})
  end
  ...
  local function handle_compile_opts(exprs, parent, opts, ast)
    ...
    if opts.target then
      ...
      emit(parent, string.format("%s = %s", opts.target, _270_()), ast)
    else
    end
  ...
  local function propagate_trace_info(_276_, _index, node)
    local _arg_277_ = _276_
    local filename = _arg_277_["filename"]
    local line = _arg_277_["line"]
    local bytestart = _arg_277_["bytestart"]
    local byteend = _arg_277_["byteend"]
  ...
  local function macroexpand_2a(ast, scope, _3fonce)
    ...
    if utils["list?"](ast) then
      _279_ = find_macro(ast, scope, utils["multi-sym?"](ast[1]))
    else
      _279_ = nil
    end
    if (_279_ == false) then
    ...
    elseif (nil ~= _279_) then
      ...
      ok, transformed = xpcall(_281_, debug.traceback)
      local function _283_()
        local _282_ = ast
        local function _284_(...)
          return propagate_trace_info(_282_, ...)
        end
        return _284_
      end
      utils["walk-tree"](transformed, _283_())
  ...
  local function compile_function_call(ast, scope, parent, opts, compile1, len)
    ...
    for i = 2, len do
      ...
      subexprs = compile1(ast[i], scope, parent, {nval = _290_})
  ...
  local function compile_varg(ast, scope, parent, opts)
    ...
    if scope.hashfn then
      ...
      _295_ = "unexpected vararg"
    end
    assert_compile(scope.vararg, _295_, ast)
  ...
  local function serialize_number(n)
    local _298_ = string.gsub(tostring(n), ",", ".")
    return _298_
  end
  local function compile_scalar(ast, _scope, parent, opts)
    local serialize
    do
      local _299_ = type(ast)
      if (_299_ == "nil") then
        serialize = tostring
      elseif (_299_ == "boolean") then
        serialize = tostring
      elseif (_299_ == "string") then
        serialize = serialize_string
      elseif (_299_ == "number") then
  ...
  local function compile_table(ast, scope, parent, opts, compile1)
    ...
    local function write_other_values(k)
      if ((type(k) ~= "number") or (math.floor(k) ~= k) or (k < 1) or (k > #ast)) then
        if ((type(k) == "string") and utils["valid-lua-identifier?"](k)) then
          ...
          local _let_301_ = compile1(k, scope, parent, {nval = 1})
          local compiled = _let_301_[1]
    ...
    do
      ...
      local function _307_(_305_)
        ...
        local k1 = _arg_306_[1]
        local k2 = _arg_306_[2]
        local _let_308_ = compile1(ast[k2], scope, parent, {nval = 1})
        local v = _let_308_[1]
        return string.format("%s = %s", k1, tostring(v))
      end
      utils.map(keys, _307_, buffer)
    end
    for i = 1, #ast do
  ...
  local function destructure(to, from, ast, scope, parent, opts)
    local opts0 = (opts or {})
    local _let_310_ = opts0
    local isvar = _let_310_["isvar"]
    local declaration = _let_310_["declaration"]
    local forceglobal = _let_310_["forceglobal"]
    local forceset = _let_310_["forceset"]
    local symtype = _let_310_["symtype"]
    ...
    local function destructure_table(left, rightexprs, top_3f, destructure1)
      ...
      do
        local _323_
        if top_3f then
          _323_ = exprs1(compile1(from, scope, parent))
        else
          _323_ = exprs1(rightexprs)
        end
        if (_323_ == "") then
          right = "nil"
  ...
  local function traceback_frame(info)
    ...
    elseif (info.what == "C") then
      ...
      if (info.what == "Lua") then
        ...
        return string.format("  %s:%d: in function %s", info.short_src, info.currentline, _342_())
  ...
  local function traceback(_3fmsg, _3fstart)
    ...
    if ((msg:find("^Compile error") or msg:find("^Parse error")) and not utils["debug-on?"]("trace")) then
      ...
      while not done_3f do
        do
          local _346_ = debug.getinfo(level, "Sln")
          if (_346_ == nil) then
            done_3f = true
  ...
  local function do_quote(form, scope, parent, runtime_3f)
    ...
    elseif utils["list?"](form) then
      ...
      mapped = utils.kvmap(form, entry_transform(_354_, q))
      local filename
      if form.filename then
        filename = string.format("%q", form.filename)
      else
    ...
    elseif utils["sequence?"](form) then
      ...
      return string.format("setmetatable({%s}, {filename=%s, line=%s, sequence=%s})", mixed_concat(mapped, ", "), filename, _357_, "(getmetatable(sequence()))['sequence']")
    elseif (type(form) == "table") then
      ...
      return string.format("setmetatable({%s}, {filename=%s, line=%s})", mixed_concat(mapped, ", "), filename, _360_())
...
package.preload["fennel.parser"] = package.preload["fennel.parser"] or function(...)
  ...
  local function parser(getbyte, _3ffilename, _3foptions)
    ...
    local function parse_error(msg, byteindex_override)
      local _let_176_ = (_3foptions or utils.root.options or {})
      local source = _let_176_["source"]
      local unfriendly = _let_176_["unfriendly"]
    ...
    local function parse_stream()
      ...
      local function dispatch(v)
        local _179_ = stack[#stack]
        if (_179_ == nil) then
          retval, done_3f, whitespace_since_dispatch = v, true, false
          return nil
        elseif ((_G.type(_179_) == "table") and (nil ~= (_179_).prefix)) then
          local prefix = (_179_).prefix
          local source
          do
            local _180_ = table.remove(stack)
            do end (_180_)["byteend"] = byteindex
            source = _180_
          end
      ...
      local function badend()
        ...
        return parse_error(string.format("expected closing delimiter%s %s", _182_, string.char(unpack(accum))))
      end
      ...
      local function parse_comment(b, contents)
        if (b and (10 ~= b)) then
          local function _186_()
            local _185_ = contents
            table.insert(_185_, string.char(b))
            return _185_
          end
          return parse_comment(getb(), _186_())
      ...
      local function add_comment_at(comments, index, node)
        local _189_ = comments[index]
        if (nil ~= _189_) then
          local existing = _189_
      ...
      local function parse_string_loop(chars, b, state)
        ...
        do
          local _199_ = {state, b}
          if ((_G.type(_199_) == "table") and ((_199_)[1] == "base") and ((_199_)[2] == 92)) then
            state0 = "backslash"
          elseif ((_G.type(_199_) == "table") and ((_199_)[1] == "base") and ((_199_)[2] == 34)) then
            state0 = "done"
          elseif ((_G.type(_199_) == "table") and ((_199_)[1] == "backslash") and ((_199_)[2] == 10)) then
      ...
      local function parse_string()
        ...
        local _203_ = (rawget(_G, "loadstring") or load)(("return " .. formatted))
        if (nil ~= _203_) then
          local load_fn = _203_
      ...
      local function parse_number(rawstr)
        ...
        if rawstr:match("^%d") then
          ...
          local _209_ = tonumber(number_with_stripped_underscores)
          if (nil ~= _209_) then
            local x = _209_
            dispatch(x)
            return true
      ...
      local function parse_sym(b)
        ...
        local source = {byteend = byteindex, bytestart = bytestart, filename = _3ffilename, line = line}
        if (rawstr == "true") then
          return dispatch(true)
        elseif (rawstr == "false") then
          return dispatch(false)
        elseif (rawstr == "...") then
          return dispatch(utils.varg(source))
        elseif rawstr:match("^:.+$") then
          return dispatch(rawstr:sub(2))
        elseif not parse_number(rawstr) then
          return dispatch(utils.sym(check_malformed_sym(rawstr), source))
        else
          return nil
        end
      end
...
local function eval(str, options, ...)
  ...
  loader = specials["load-code"](lua_source, env, _663_(...))
  opts.filename = nil
  return loader(...)
end
...
local function syntax()
  ...
  for k, v in pairs(_G) do
    local _664_ = type(v)
    if (_664_ == "function") then
      out[k] = {["global?"] = true, ["function?"] = true}
    elseif (_664_ == "table") then
...
do
  ...
  package.preload[module_name] = _667_
  _ = nil
  local env
  do
    local _668_ = specials["make-compiler-env"](nil, compiler.scopes.compiler, {})
    do end (_668_)["utils"] = utils
    _668_["fennel"] = mod
    env = _668_
  end
